...
export function composeSummary(opts: {
  ...
  if (opts.profileKey === "ai_engineer" || opts.profileKey === "applied_ai") {
    ...
    text = `${title} with hands-on experience designing and implementing full-stack product interfaces using React and TypeScript. Shipped verified product work including ${names.slice(0, 2).join(" and ")}. Comfortable owning features from interaction design through API integration and production deployment.`;
...
export function composeSkills(opts: {
  ...
  const byName = new Map(opts.inventory.skills.filter((s) => s.approvedForCV && s.verified).map((s) => [s.name.toLowerCase(), s]));
  ...
  // Generic fallback — keep categories from inventory
  const limit = opts.pageLength === 1 ? 12 : 16;
  const scored = opts.inventory.skills
    .filter((s) => s.approvedForCV && s.verified)
    .filter((s) => s.profiles.includes("*") || s.profiles.includes(opts.profileKey))
    .map((s) => {
      const aliases = [s.name, ...s.keywords];
      const inJd = aliases.some((a) => opts.jobCorpus.includes(a.toLowerCase()));
      const priority = policy.skillPriority.findIndex((p) => p.toLowerCase() === s.name.toLowerCase());
